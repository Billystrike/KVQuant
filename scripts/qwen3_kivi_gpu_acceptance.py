...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Qwen3-8B KIVI GPU acceptance")
...
def main() -> None:
    ...
    with torch.inference_mode():
        ...
        kivi_prefill_cache = Qwen3KiviCache(cache_config)
        candidate = model(
            input_ids=input_ids,
            attention_mask=attention_mask,
            past_key_values=kivi_prefill_cache,
            use_cache=True,
            logits_to_keep=1,
        )
    prefill_logits_exact = torch.equal(candidate.logits, reference.logits)
    prefill_logits_max_abs_delta = float((candidate.logits - reference.logits).abs().max().item())
    key_storage_changed_layers = sum(
        not torch.equal(kivi_prefill_cache.key_cache[i], reference_cache.key_cache[i])
        for i in range(EXPECTED["num_hidden_layers"])
    )
    value_storage_changed_layers = sum(
        not torch.equal(kivi_prefill_cache.value_cache[i], reference_cache.value_cache[i])
        for i in range(EXPECTED["num_hidden_layers"])
    )
    prefill_key_quantized_lengths = list(kivi_prefill_cache.key_quantized_lengths)
    prefill_value_quantized_lengths = list(kivi_prefill_cache.value_quantized_lengths)
    del reference, candidate, reference_cache, kivi_prefill_cache
    torch.cuda.empty_cache()

    generation_cache = Qwen3KiviCache(cache_config)
    ...
    source_hashes = {
        ...
        "qwen3_kivi_gpu_acceptance_source_sha256": _sha256(Path(__file__).resolve()),
    }
    ...
    expected_key_quantized_length = (
        expected_resume_cache_length - expected_resume_cache_length % args.residual_length
    )
    expected_value_quantized_length = max(0, expected_resume_cache_length - args.residual_length)
    think_match = re.search(r"<think>(.*?)</think>", rendered_prompt, flags=re.DOTALL)
    no_persistent_bucket_indices = not hasattr(generation_cache, "key_bucket_indices") and not hasattr(
        generation_cache, "value_bucket_indices"
    )
    ...
    checks = {
        ...
        "key_storage_changed_all_layers": key_storage_changed_layers
        == EXPECTED["num_hidden_layers"],
        "value_storage_changed_all_layers": value_storage_changed_layers
        == EXPECTED["num_hidden_layers"],
        ...
        "layer_count": len(generation_cache.key_cache) == EXPECTED["num_hidden_layers"],
        "no_persistent_bucket_indices": no_persistent_bucket_indices,
    ...
    report = {
        ...
        "key_storage_changed_layers": key_storage_changed_layers,
        "value_storage_changed_layers": value_storage_changed_layers,
        ...
        "layer_count": len(generation_cache.key_cache),
        "no_persistent_bucket_indices": no_persistent_bucket_indices,
