"""Run deterministic native-context FP16 passkey calibration cases."""
...
def validate_native_model_context(config: Any, model: dict[str, Any]) -> None:
    if getattr(config, "model_type", None) != "llama":
        raise PasskeyError("passkey Stage A requires model_type == 'llama'")
...
def _load_model(model_config: dict[str, Any], config: Any):
...
def _attach_quality_gate(
    ...
    for prompt_length in manifest["prompt_lengths"]:
        for position_percent in manifest["passkey_positions_percent"]:
            ...
            cells.append({
                ...
                "exact_matches": sum(record["generation"]["exact_match"] for record in cell),
                "completed_cases": len(cell),
                "expected_cases": manifest["key_generation"]["count"],
            })
    result["quality_cells"] = cells
    complete = (
        len(records)
    ...
    if manifest["key_generation"]["count"] == 1:
        passed = all(cell["exact_matches"] == 1 for cell in cells)
    else:
        passed = (
            result["exact_match_accuracy"] >= 0.9
            and all(cell["exact_matches"] >= 4 for cell in cells)
        )
...
def run_manifest(manifest_path: str | Path) -> tuple[int, dict[str, Any]]:
    ...
    try:
        ...
        if source_state["dirty"]:
            raise PasskeyError(
                "formal passkey calibration requires a clean tracked source state"
            )
        config, tokenizer = _load_preflight(manifest["model"])
    ...
    pending = [case for case in cases if case["case_id"] not in set(reusable)]
    completed_before_run = aggregate_passkey_cases(
        output_dir, expected_case_ids=expected_ids
    )
    result: dict[str, Any] = {
        "git_commit": source_state["git_commit"],
        "source_dirty": source_state["dirty"],
        "generated_keys": ",".join(generated_keys),
        "native_max_position_embeddings": config.max_position_embeddings,
        "native_rope_scaling": getattr(config, "rope_scaling", None),
        "expanded_cases": len(cases),
        "valid_reusable_cases": len(reusable),
        "remaining_cases": len(pending),
        "completed_cases": len(completed_before_run),
        "failure_records": sum(
            (output_dir / "failures" / f"{case_id}.json").is_file()
            for case_id in expected_ids
        ),
    }
    _attach_quality_counts(result, completed_before_run)
    _attach_quality_gate(result, manifest, completed_before_run)
    ...
    try:
        model = _load_model(manifest["model"], config)
    except torch.cuda.OutOfMemoryError as error:  # pragma: no cover - environment dependent.
        ...
        for case in pending:
            try:
                ...
                atomic_write_json(
                    ...
                    _failure_record(case["case_id"], "cuda_out_of_memory", "generation", error),
                )
                ...
                _attach_quality_gate(result, manifest, completed)
                _write_quality_summary(output_dir, result)
                return EXIT_OOM, {**result, "error": str(error), "stage": "generation"}
            except Exception as error:
                case_failed = True
                atomic_write_json(
                    output_dir / "failures" / f"{case['case_id']}.json",
                    _failure_record(case["case_id"], "passkey_case_error", "generation", error),
                )
    finally:
        del model
        gc.collect()
        if manifest["model"]["device"] == "cuda":
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    for cell in result.get("quality_cells", []):
        print(
            f"cell_exact[{cell['prompt_length']},{cell['position_percent']}]="
