...
from utils.qwen3_cage_v3_screen import PARTITIONS, SCIENTIFIC_FIELDS


def _payload(root: Path, *, partition: str) -> tuple[dict, list[dict]]:
    summary = load_json(root / "summary.json")
    expected = 12 if partition == "cage_qwen3" else 3
    if (
        summary.get("status") != "pass"
        or summary.get("stage") != "screen_acceptance"
        ...
        raise RuntimeError(f"incomplete CAGE-v3 screen acceptance: {root}")
    records = []
    for path in sorted((root / "cases").glob("*.json")):
        value = load_json(path)
        if value.get("status") != "completed" or path.stem != value.get("case_id"):
            raise RuntimeError(f"invalid screen acceptance case: {path}")
        records.append({field: value[field] for field in SCIENTIFIC_FIELDS})
    if len(records) != expected:
        raise RuntimeError(f"screen acceptance case count mismatch: {root}")
    records.sort(key=lambda record: record["case_id"])
    return summary, records


def main() -> None:
    parser = argparse.ArgumentParser(description="Compare fresh CAGE-v3 screen acceptance repeats")
    ...
    first_root = args.repeat_a.resolve()
    second_root = args.repeat_b.resolve()
    if first_root == second_root:
        raise RuntimeError("screen acceptance repeat directories must differ")
    first_summary, first = _payload(first_root, partition=args.partition)
    second_summary, second = _payload(second_root, partition=args.partition)
    if first_summary["identity"] != second_summary["identity"]:
        raise RuntimeError("screen acceptance identities differ")
    ...
    payload_sha256 = canonical_sha256(first)
    result = {
        "schema_version": 1,
        ...
        "repeat_a": str(first_root),
        "repeat_b": str(second_root),
    }
    output = args.output.resolve()
    if output.exists():
        raise FileExistsError(f"refusing to overwrite screen comparison: {output}")
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps(result, indent=2, sort_keys=True, allow_nan=False) + "\n", encoding="utf-8")
    print(json.dumps(result, indent=2, sort_keys=True, allow_nan=False))
    if mismatches:
        raise RuntimeError(f"CAGE-v3 screen acceptance mismatches: {mismatches}")
