...
def main() -> None:
    parser = argparse.ArgumentParser(description="Jointly validate the 75-case CAGE-v3 development screen")
    ...
    output = {
        "schema_version": 1,
        "status": "pass",
        "audit_id": "qwen3-8b-cage-v3-development-screen-postrun-v1",
    ...
    if output["case_count"] != 75 or output["layer_record_count"] != 2700:
        raise RuntimeError("joint CAGE-v3 postrun totals mismatch")
    output_path = args.output.resolve()
    if output_path.exists():
        raise FileExistsError(f"refusing to overwrite CAGE-v3 postrun audit: {output_path}")
