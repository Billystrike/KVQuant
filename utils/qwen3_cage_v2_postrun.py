...
from utils.qwen3_cage_v2_gate import canonical_sha256, file_sha256, load_json
...
ARTIFACT_SET_ID = "qwen3-8b-cage-v2-round1-development-screen-artifacts-v1"
...
class CageV2PostrunError(RuntimeError):
    pass


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise CageV2PostrunError(message)
...
def validate_artifact_manifest(
    ...
    *,
    ...
    _require(manifest.get("protocol_sha256") == protocol_sha256, "artifact protocol hash mismatch")
    _require(manifest.get("manifest_sha256") == input_manifest_sha256, "artifact input hash mismatch")
    _require(manifest.get("acceptance_gate_sha256") == gate_sha256, "artifact gate hash mismatch")
    _require(tuple(manifest.get("partitions", {})) == PARTITIONS, "artifact partitions mismatch")
    expected_counts = {"cage_qwen3": 115, "kitty_qwen3": 30}
    for partition, count in expected_counts.items():
        record = manifest["partitions"][partition]
        _require(record.get("case_count") == count, f"{partition} artifact case count mismatch")
        _require(record.get("layer_record_count") == count * 36, f"{partition} layer count mismatch")
        _require(record.get("failure_count") == 0, f"{partition} artifact declares failures")
        for field in (
            "case_ids_sha256",
            "run_identity_sha256",
            "summary_sha256",
...
def _validate_cache(record: dict[str, Any]) -> None:
    ...
    if name == "kitty":
        ...
        _require(cache.get("official_config") == expected, "Kitty official configuration mismatch")
        return
    ...
    if name == "cage_v1":
        ...
        _require(cache.get("value_adaptive") is False, "CAGE-v2 Value adaptation changed")
        ...
        raise CageV2PostrunError(f"unsupported screen method: {name}")
...
def validate_partition(
    ...
    *,
    ...
    _require("SCREEN END ===" in log_text, f"{partition} log lacks completion marker")

    identity = load_json(identity_path)
    summary = load_json(summary_path)
    count = spec["case_count"]
    ...
    _require(
        identity.get("expected_case_ids") == list(expected_cases),
        f"{partition} ordered expected case IDs mismatch",
    )
    ...
    _require(source.get("git_commit") == accepted_source["cage_commit"], f"{partition} execution commit mismatch")
    _require(source.get("dirty") is False, f"{partition} source was dirty")
    if partition == "kitty_qwen3":
        _require(source.get("kitty_commit") == accepted_source["kitty_commit"], "Kitty commit mismatch")
        _require(source.get("transformers_commit") == accepted_source["transformers_commit"], "Transformers mismatch")
    ...
    for model_key, source_key in (
        ...
        _require(model["source_sha256"].get(model_key) == accepted_source[source_key], f"{partition} source mismatch: {model_key}")
    ...
    _require(shell_case_manifest_sha256(paths) == spec["shell_case_file_manifest_sha256"], f"{partition} shell manifest mismatch")
    ...
    _require(set(records) == set(expected_cases), f"{partition} expected case set mismatch")
    _require(layer_count == spec["layer_record_count"], f"{partition} total layer count mismatch")
